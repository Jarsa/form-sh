...
class PointOfSale(TransactionCase):
    def setUp(self):
        ...
        self.env.user.groups_id |= self.env.ref("point_of_sale.group_pos_manager") | self.env.ref(
    ...
    def base_test(self, pac_name):
        ...
        self.env.company.l10n_mx_edi_pac = pac_name or "finkok"
        ...
        self.assertFalse(
            self.att_obj.search([("res_model", "=", "pos.session"), ("res_id", "=", self.session.id)]),
        ...
        self.assertTrue(order.account_move, "Invoice not validated")
        self.assertEqual(
            order.account_move.l10n_mx_edi_payment_method_id, self.env.ref("l10n_mx_edi.payment_method_efectivo")
        )
        self.assertEqual(
            order.account_move.l10n_mx_edi_usage, order.l10n_mx_edi_usage, "Usage not assigned to the invoice"
        )
        ...
        self.session.action_pos_session_closing_control()
        self.assertEqual(self.session.l10n_mx_edi_pac_status, "signed", self.session.message_ids.mapped("body"))
        self.assertEqual(
            len(
                self.att_obj.search(
                    [("res_model", "=", "pos.session"), ("res_id", "=", self.session.id), ("name", "like", "%.xml")]
                ).ids
            ),
            1,
        ...
        self.assertEqual(self.session.l10n_mx_edi_pac_status, "retry", self.session.message_ids.mapped("body"))
        self.company.l10n_mx_edi_pac = pac_name or "finkok"
        ...
        self.assertTrue(
            self.session.l10n_mx_edi_pac_status in ["cancelled", "to_cancel"], self.session.message_ids.mapped("body")
        )
        ...
        self.assertEqual(
            round(order.amount_total, 2), float(xml_total), "The amount with include base amount is incorrect"
        )

    def test_001_l10n_mx_edi_pos(self):
        """Make sure all POS Flow is possible with the main pac: finkok"""
        self.base_test("finkok")
    ...
    def test_002_create_credit_note_previous_open_invoice(self):
        ...
        payment = self.pmp_obj.with_context(active_id=order.id).create(
            {"payment_method_id": order.session_id.config_id.payment_method_ids[0].id, "amount": order.amount_total}
        )
        payment.check()
        ...
        self.assertEqual(
            (refund.account_move.state, refund.account_move.move_type),
            ("posted", "out_refund"),
    ...
    def test_004_refund_closed_session(self):
        ...
        # close session
        self.session.action_pos_session_closing_control()
        self.assertEqual(self.session.state, "closed", "The session 1 is not closed")
        self.assertEqual(self.session.l10n_mx_edi_pac_status, "signed", self.session.message_ids.mapped("body"))
        # create session 2
        ...
        self.assertEqual(session2.l10n_mx_edi_pac_status, "signed", session2.message_ids.mapped("body"))

        filename = session2.order_ids.get_file_name()
        attachment = session2.l10n_mx_edi_retrieve_last_attachment("%s.xml" % filename)
        self.assertTrue(attachment, "attachment not created")
        xml = fromstring(base64.b64decode(attachment.datas))
        self.assertEqual(xml.get("TipoDeComprobante"), "E", "The refund is not set as an outcome")
    ...
    def test_005_no_consider_refund_without_cfdi_origin(self):
        ...
        self.assertEqual(refund.state, "paid", "The refund order was not paid")
        session2.action_pos_session_closing_control()
        self.assertEqual(session2.l10n_mx_edi_pac_status, "retry", session2.message_ids.mapped("body"))
        expected_message = "The following orders were skipped because it's not"
        self.assertIn(expected_message, "".join(session2.message_ids.mapped("body")))
        attachments = self.att_obj.search([("res_model", "=", "pos.session"), ("res_id", "=", session2.id)])
        self.assertFalse(attachments, "attachment created")
        self.session.l10n_mx_edi_update_pac_status()
        # TODO: Why is this not being sing after this change. Maybe was skipped becase the error and this really
        # the correct expected The attribute 'Rfc' is required but missing.
        self.assertEqual(self.session.l10n_mx_edi_pac_status, "retry", self.session.message_ids.mapped("body"))
        expected_message = "The following orders were skipped because it's not"
        self.assertIn(expected_message, "".join(self.session.message_ids.mapped("body")))
        # attachments = self.att_obj.search([('res_model', '=', 'pos.session'), ('res_id', '=', self.session.id)])
        # self.assertFalse(attachments, 'Attachment created: %s' % attachments.name)
    ...
    def test_007_l10n_mx_edi_pos(self):
        """Make sure all POS Flow is possible with Smart Web"""
        # Credentials needed for Smart Web
        self.env.company.l10n_mx_edi_pac_username = "luis_t@vauxoo.com"
        self.env.company.l10n_mx_edi_pac_password = "VAU.2021.SW"
        self.base_test("sw")
    ...
    def test_tax_include(self):
        ...
        attach = fromstring(base64.b64decode(attach))
        xml_total = attach.get("Total")
        self.assertEqual(round(order.amount_total, 2), float(xml_total), "The amount with price include is incorrect")
    ...
    def test_both_taxes0(self):
        ...
        attach = fromstring(base64.b64decode(attach))
        self.assertEqual(len(attach.Conceptos.Concepto.Impuestos.Traslados.Traslado), 2, "Missing taxes on concept")
    ...
    def create_session(self):
        ...
        config.payment_method_ids[0].l10n_mx_edi_payment_method_id = self.env.ref(
            "l10n_mx_edi.payment_method_efectivo"
        )
        config.sudo().available_pricelist_ids.currency_id = config.sudo().currency_id
        return (
    ...
    def pay_order(self, order):
        payment = (
            self.pmp_obj.with_env(self.env(user=self.uid))
