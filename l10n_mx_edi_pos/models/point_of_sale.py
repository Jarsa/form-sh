...
class PosSession(models.Model):
    ...
    l10n_mx_edi_pac_status = fields.Selection(
        selection=[("retry", "Retry"), ("signed", "Signed"), ("to_cancel", "To cancel"), ("cancelled", "Cancelled")],
        string="PAC status",
    ...
    def _l10n_mx_edi_retry(self):
        ...
        for order in orders:
            ...
            if (order.amount_total < 0 and not related.l10n_mx_edi_uuid) or (
                ...
                and related.session_id.state == "closed"
            ):
                skip_orders |= order
        orders -= skip_orders
        if skip_orders:
            ...
            msg_body = (
                _(
                    """The following orders were skipped because it's not
        ...
        lambda_functions = (
            ...
            and r.partner_id  # Order with partner
            and r.partner_id.commercial_partner_id.id not in partners.ids,
            lambda r: r.amount_total > 0 and not r.partner_id,  # Order without Partner
            lambda r: r.amount_total < 0
            and r.partner_id  # Refund with partner
            and (r.partner_id.commercial_partner_id.id not in partners.ids or not r.invoice_id),
        ...
        for func in lambda_functions:
            ...
            self.message_post(body=_("CFDI document generated (may be not signed)"), attachment_ids=[attachment_id.id])
    ...
    def _l10n_mx_edi_call_service(self, service_type, cfdi):
        ...
        if service_type == "sign":
            return getattr(edi, "_l10n_mx_edi_%s_%s_invoice" % (pac_name, service_type))(
                self.move_id, credentials, cfdi
            )
        uuid = self.move_id._l10n_mx_edi_decode_cfdi(cfdi).get("uuid")
        return getattr(edi, "_l10n_mx_edi_%s_%s_service" % (pac_name, service_type))(uuid, company_id, credentials)
    ...
    def _l10n_mx_edi_post_sign_process(self, cfdi_values, order_ids):
        ...
        if xml_signed:
            ...
            order_ids.write({"l10n_mx_edi_cfdi_generated": True, "l10n_mx_edi_uuid": uuid})
        else:
    ...
    def _l10n_mx_edi_post_cancel_process(self, cfdi_values, order_ids, attach):
        ...
        self.message_post(body=body_msg + create_list_html(cfdi_values.get("errors")))
    ...
    def l10n_mx_edi_retrieve_attachments(self, filename):
        ...
        """
        self.ensure_one()
        domain = [("res_id", "=", self.id), ("res_model", "=", self._name), ("name", "=", filename)]
    ...
    def l10n_mx_edi_amount_to_text(self, amount_total):
        ...
        words = (
            self.currency_id.with_context(lang=self.company_id.partner_id.lang or "es_ES")
        ...
        for record in self:
            ...
            for att in attach_xml_ids.filtered("datas"):
                ...
                try:
                    ...
                    continue
                record.l10n_mx_edi_sat_status = CFDI_SAT_QR_STATE.get(status[0] if status else "", "none")
...
class PosOrder(models.Model):
    ...
    l10n_mx_edi_cfdi_generated = fields.Boolean(
        "XML Generated", copy=False, help="Indicate if this order was consider in the session XML"
    )
    l10n_mx_edi_uuid = fields.Char(
        "Fiscal Folio", copy=False, index=True, help="Folio in electronic document, returned by SAT."
    )
    ...
    def get_file_name(self):
        ...
        if count > 0:
            fname = "%s_%s%s_%s" % (session_name, type_rec, egre, count + 1)
        return fname

    def _get_subtotal_wo_discount(self, precision_digits, line):
        return float(
            line.price_subtotal / (1 - abs(line.discount / 100))
            if line.discount != 100
            else abs(line.price_unit * line.qty)
        )

    def _get_discount(self, precision_digits, line):
        return (
            float(self._get_subtotal_wo_discount(precision_digits, line) - abs(line.price_subtotal))
            if line.discount
            else False
        )
    ...
    def _l10n_mx_edi_create_cfdi_values(self):
        ...
        invoice["subtotal_wo_discount"] = "%.*f" % (
            precision_digits,
            sum(self._get_subtotal_wo_discount(precision_digits, line) for line in self.mapped("lines")),
        )
        invoice["amount_untaxed"] = abs(
            float_round(sum(self._get_subtotal_wo_discount(precision_digits, p) for p in self.mapped("lines")), 2)
        )
        invoice["amount_discount"] = "%.*f" % (
            precision_digits,
            sum(float(self._get_discount(precision_digits, p)) for p in self.mapped("lines")),
        )

        invoice["tax_name"] = lambda t: {"ISR": "001", "IVA": "002", "IEPS": "003"}.get(t, False)
        invoice["taxes"] = self._l10n_mx_edi_create_taxes_cfdi_values()

        invoice["amount_total"] = abs(
            float_round(float(invoice["amount_untaxed"]), 2)
            - round(float(invoice["amount_discount"] or 0), 2)
            + (round(invoice["taxes"]["total_transferred"] or 0, 2))
            - (round(invoice["taxes"]["total_withhold"] or 0, 2))
        )
        invoice["document_type"] = "I" if self.filtered(lambda r: r.amount_total > 0) else "E"
        payment_ids = self.mapped("payment_ids").filtered(lambda st: st.amount > 0)

        # Get highest amount payment
        payments = payment_ids.read_group([("id", "in", payment_ids.ids)], ["amount"], "payment_method_id")
        ...
        invoice["payment_method"] = (
            self.env["pos.payment.method"].browse(payment_id).l10n_mx_edi_payment_method_id.code
            if payment_id
            else "99"
        )
    ...
    def get_cfdi_related(self):
        ...
        Considered:
            ...
            - Order Cancelled"""
        cfdi_related = []
        refund = self.filtered(lambda a: a.amount_total < 0 and not a.l10n_mx_edi_uuid)
        relation_type = "04" if self[0].l10n_mx_edi_uuid else ("01" if refund else "")
        for order in refund:
            origin = self.search(
                [
    ...
    def _l10n_mx_edi_create_taxes_cfdi_values(self):
        ...
        for line in self.mapped("lines").filtered("price_subtotal"):
            ...
            tax_line = {
                tax["id"]: tax
                for tax in line.tax_ids_after_fiscal_position.compute_all(
                    price,
            ...
            for tax in line.tax_ids_after_fiscal_position.filtered(lambda r: r.l10n_mx_tax_type != "Exento"):
                ...
                base = (
                    ...
                    else tax_dict.get("base", line.price_subtotal)
                )
    ...
    def _prepare_invoice_vals(self):
        ...
        payment_ids = self.mapped("payment_ids").filtered(lambda st: st.amount > 0)
        payment_method_ids = payment_ids.read_group(
            [("id", "in", payment_ids.ids)], ["payment_method_id"], "payment_method_id"
        )
        max_count = 0
        payment_method_id = False
        for payment_method in payment_method_ids:
            if payment_method.get("payment_method_id_count") > max_count:
                max_count = payment_method.get("payment_method_id_count")
                payment_method_id = payment_method.get("payment_method_id")[0]
        payment_method = self.env["pos.payment.method"].browse(payment_method_id)
        if payment_method.l10n_mx_edi_payment_method_id:
            res["l10n_mx_edi_payment_method_id"] = payment_method.l10n_mx_edi_payment_method_id.id
