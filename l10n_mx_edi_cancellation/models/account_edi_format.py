...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_mx_edi_finkok_cancel(self, move, credentials, cfdi):
        ...
        cancellation_data = (move.l10n_mx_edi_cancellation or move.payment_id.l10n_mx_edi_cancellation or "").split(
            "|"
        )
        try:
            ...
            if (
                ...
                and move.l10n_mx_edi_cancel_invoice_id
                and move.l10n_mx_edi_cancel_invoice_id != move
            ):
        ...
        if move.l10n_mx_edi_cancellation != "01":
            related = self._l10n_mx_edi_finkok_get_documents_related(
                move, credentials["username"], credentials["password"], uuid, cer_pem, key_pem
            )
        if related:
            return {
        ...
        if not status or status == "None":
            return {
                "errors": [
                    _(
                        "The PAC has sent this document to cancel, but the SAT has not processed it yet. Please try "
                        "again in a few minutes or wait for the automatic action. SAT status for cancellation: %s",
    ...
    def _l10n_mx_edi_finkok_get_documents_related(self, move, username, password, uuid, cer, key):
        ...
        sat_status = self._l10n_mx_edi_finkok_get_status(
            move, username, password, supplier_rfc, move.l10n_mx_edi_cfdi_customer_rfc, uuid, move.amount_total
        )
