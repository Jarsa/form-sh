...
class AccountMove(models.Model):
    ...
    def invoice_validate(self):
        for record in self.l10n_mx_edi_cfdi_request in ("on_invoice", "on_refund"):
            if record.invoice_line_ids.filtered(lambda r: r.l10n_mx_edi_voucher_id and r.quantity != 0):
                record.message_post(
                    body=_(
...
class AccountMoveLine(models.Model):
    ...
    l10n_mx_edi_voucher_id = fields.Many2one(
        "res.partner",
        string="Employee",
        help="Employee information, set this if you want to use the Food Voucher Complement",
    )
