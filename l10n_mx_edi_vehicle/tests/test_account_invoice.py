...
class TestL10nMxEdiInvoiceVehicle(TestMxEdiCommon):
    ...
    def test_l10n_mx_edi_invoice_cd(self):
        ...
        namespaces = {"destruccion": "http://www.sat.gob.mx/certificadodestruccion"}
        comp = xml.Complemento.xpath("//destruccion:certificadodedestruccion", namespaces=namespaces)
    ...
    def test_l10n_mx_edi_xsd(self):
        ...
        xsd_file = self.ref("l10n_mx_edi.xsd_cached_certificadodedestruccion_xsd")
    ...
    def test_invoice_renew_and_substitution(self):
        ...
        vehicle_type_tag = self.env["fleet.vehicle.tag"].search([("name", "=", "01 - Fifth wheel tractor")])
        if not vehicle_type_tag:
            vehicle_type_tag = self.env["fleet.vehicle.tag"].create({"name": "01 - Fifth wheel tractor"})
        substitute_vehicle_id.tag_ids.unlink()
        substitute_vehicle_id.tag_ids = [self.env.ref("fleet.vehicle_3").sudo().tag_ids[0].id, vehicle_type_tag.id]
        ...
        self.assertEqual(invoice.edi_state, "sent", invoice.message_ids.mapped("body"))
        xml = objectify.fromstring(generated_files[0])
        namespaces = {"decreto": "http://www.sat.gob.mx/renovacionysustitucionvehiculos"}
        comp = xml.Complemento.xpath("//decreto:renovacionysustitucionvehiculos", namespaces=namespaces)  # noqa
        self.assertTrue(comp, "Complement to Renew ans Substitution not added correctly")
    ...
    def test_pfic(self):
        ...
        comp = xml.Complemento.xpath("//pfic:PFintegranteCoordinado", namespaces=namespaces)
    ...
    def xml2dict(self, xml):
        ...
        def recursive_dict(element):
            return (
                element.tag,
                dict(
                    (recursive_dict(e) for e in element.getchildren()),
                    ____text=(element.text or "").strip(),
                    **element.attrib
                ),
            )
