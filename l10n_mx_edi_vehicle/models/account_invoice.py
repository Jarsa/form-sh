...
class AccountMove(models.Model):
    ...
    l10n_mx_edi_serie_cd = fields.Selection(
        ...
        "Serie",
        help="Assign the serie according the SAT catalog to the vehicle that was destroyed",
    )
    l10n_mx_edi_folio_cd = fields.Char("Folio", help="Assign the folio provided by the SAT to this destruction")
    ...
    l10n_mx_edi_vehicle_ids = fields.Many2many(
        "fleet.vehicle", string="Used Vehicles", help="Indicate the vehicles that are going to be replaced"
    )
    l10n_mx_edi_substitute_id = fields.Many2one(
        "fleet.vehicle", "Substitute Vehicle", help="Indicate the vehicle that is going to be replaced"
    )
    l10n_mx_edi_complement_type = fields.Selection(related="company_id.l10n_mx_edi_complement_type", readonly=True)
    ...
    def _l10n_mx_edi_decode_cfdi(self, cfdi_data=None):
        ...
        if not cfdi_data:
            return result
        cfdi_data = (
            cfdi_data.replace(b"xmlns__destruccion", b"xmlns:destruccion")
            .replace(b"xmlns__ventavehiculos", b"xmlns:ventavehiculos")
            .replace(b"xmlns__ventavehiculos", b"xmlns:ventavehiculos")
            .replace(b"xmlns__pfic", b"xmlns:pfic")
            .replace(b"xmlns__decreto", b"xmlns:decreto")
            .replace(b"xmlns__vehiculousado", b"xmlns:vehiculousado")
        )
