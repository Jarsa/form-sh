...
class TestL10nMxEdiRefund(TestMxEdiCommon):
    ...
    def test_l10n_mx_edi_invoice_refund(self):
        ...
        refund = self.refund_model.with_context(active_model="account.move", active_ids=self.invoice.ids).create(
            {
        ...
        self.assertEqual(
            self.payment_inmediate,
            refund.invoice_payment_term_id,
        ...
        self.assertEqual(
            "G02", refund.l10n_mx_edi_usage, "Use in refund is different to selected from the wizard of credit note"
        )
        self.assertEqual(
            "03", refund_type, "Invoice origin type is different to selected from the wizard of credit note"
        )
