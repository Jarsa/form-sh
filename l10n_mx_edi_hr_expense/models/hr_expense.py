...
class HrExpenseSheet(models.Model):
    ...
    def _get_user_domain(self):
        users = self.env["hr.employee"].search([("expense_manager_id", "!=", False)]).mapped("expense_manager_id")
    ...
    l10n_mx_edi_accountant = fields.Many2one(
        ...
        store=True,
        inverse="_inverse_accountant",
        help="This user will be the responsible to review the expenses report "
        "after the manager actually approve it.",
    )
    l10n_mx_edi_expenses_count = fields.Integer("Number of Expenses", compute="_compute_expenses_count")
    l10n_mx_edi_open_expenses_count = fields.Integer("Number of Open Expenses", compute="_compute_expenses_count")
    l10n_mx_edi_paid_expenses_count = fields.Integer("Number of Paid Expenses", compute="_compute_expenses_count")
    l10n_mx_edi_paid_invoices_count = fields.Integer("Number of Paid Invoices", compute="_compute_invoices_count")
    l10n_mx_edi_invoices_count = fields.Integer("Number of Open Invoices", compute="_compute_invoices_count")
    ...
    def _compute_expenses_count(self):
        for sheet in self:
            paid = sheet.expense_line_ids.filtered(lambda exp: exp.state == "done")
            sheet.l10n_mx_edi_expenses_count = len(sheet.expense_line_ids)
            sheet.l10n_mx_edi_paid_expenses_count = len(paid)
            sheet.l10n_mx_edi_open_expenses_count = len(sheet.expense_line_ids - paid)

    def _compute_invoices_count(self):
        for sheet in self:
            invoices = sheet.expense_line_ids.mapped("l10n_mx_edi_invoice_id")
            sheet.l10n_mx_edi_paid_invoices_count = len(
                invoices.filtered(lambda inv: inv.payment_state in ["in_payment", "paid"])
            )
            sheet.l10n_mx_edi_invoices_count = len(
                invoices.filtered(
                    lambda inv: inv.state in ("posted", "draft") and inv.payment_state not in ("in_payment", "paid")
                )
            )
    ...
    def _compute_l10n_mx_edi_accountant(self):
        ...
        for sheet in self:
            ...
            supplier = sheet.expense_line_ids.mapped("partner_id.commercial_partner_id")
            company = sheet.company_id
            currency = sheet.expense_line_ids.mapped("currency_id")
            sheet.l10n_mx_edi_accountant = (
                employee.l10n_mx_edi_accountant
                if len(supplier) != 1 or label not in supplier.category_id
                else (
                    supplier.accountant_company_currency_id
                    if currency == company.currency_id
                    else supplier.accountant_foreign_currency_id
                )
            ) or (
                company.accountant_company_currency_id
                if currency == company.currency_id
                else company.accountant_foreign_currency_id
            )
    ...
    def _name_search(self, name, args=None, operator="ilike", limit=100, name_get_uid=None):
        ...
        sheet_ids = self._search(expression.AND([domain, args]), limit=limit, access_rights_uid=name_get_uid)
    ...
    def l10n_mx_edi_create_invoice(self):
        for sheet in self:
            ...
            if not with_cfdi or all(
                inv.payment_state in ("in_payment", "paid") for inv in with_cfdi.mapped("l10n_mx_edi_invoice_id")
            ):
        ...
        return {
            ...
            "domain": [
                (
                    "id",
                    "in",
                    self.expense_line_ids.mapped("l10n_mx_edi_invoice_id")
                    .filtered(lambda inv: inv.state in ("posted", "draft"))
                    .ids,
                )
            ],
            "context": {"move_type": "in_invoice", "search_default_group_by_partner_id": 1},
        }
        ...
        return {
            ...
            "domain": [
                (
                    ...
                    "in",
                    self.expense_line_ids.mapped("l10n_mx_edi_invoice_id")
                    .filtered(lambda inv: inv.payment_state in ("in_payment", "paid"))
                    .ids,
                )
            ],
            "context": {"move_type": "in_invoice", "search_default_group_by_partner_id": 1},
    ...
    def action_get_expenses_paid_view(self):
        ...
        return {
            ...
            "view_id": False,
            "type": "ir.actions.act_window",
            "domain": [("id", "in", self.expense_line_ids.filtered(lambda exp: exp.state == "done").ids)],
        }
    ...
    def activity_update(self):
        ...
        for expense_report in self.filtered(lambda sh: sh.state == "approve" and sh.l10n_mx_edi_accountant):
            self.activity_schedule(
                "hr_expense.mail_act_expense_approval", user_id=expense_report.sudo().l10n_mx_edi_accountant.id
            )
        self.filtered(lambda hol: hol.state == "post").activity_feedback(["hr_expense.mail_act_expense_approval"])
        return res
    ...
    def approve_expense_sheets(self):
        ...
        if not self.user_has_groups("hr_expense.group_hr_expense_manager"):
            ...
            managers = self.env["hr.department"].search([]).mapped("manager_id.user_id")
            ...
            if self.env.user not in current_managers | other_managers and not self.user_has_groups(
                "l10n_mx_edi_hr_expense.allow_to_approve_exp_wo_being_responsible"
            ):
    ...
    def action_submit_sheet(self):
        ...
        for expense in self.mapped("expense_line_ids").filtered(lambda exp: exp.total_amount_company > limit):
            users = expense.company_id.l10n_mx_edi_employee_ids.mapped("user_id")
            for user in users:
...
class HrExpense(models.Model):
    ...
    active = fields.Boolean(
        help="In the line this will be necessary to split the expenses once they are received.",
        tracking=True,
        default=True,
    )
    l10n_mx_edi_functionally_approved = fields.Boolean(
        ...
        copy=False,
        help="Comply with the functional checks?",
        tracking=True,
        default=False,
        readonly=1,
    )
    l10n_mx_edi_fiscally_approved = fields.Boolean(
        "Fiscally Approved",
        copy=False,
        help="Comply with the fiscal checks?",
        tracking=True,
        default=False,
        readonly=1,
    )
    l10n_mx_edi_forced_approved = fields.Boolean(
        "Forced Approved",
        copy=False,
        help="This is a paid not deductible",
        tracking=True,
        default=False,
        readonly=1,
    )
    name = fields.Char(
        states={"downloaded": [("readonly", False)], "draft": [("readonly", False)], "refused": [("readonly", False)]}
    )
    ...
    l10n_mx_edi_uuid = fields.Text(
        "UUID", tracking=True, help="UUID of the xml's attached comma separated if more than one."
    )
    total_amount = fields.Monetary(inverse="_inverse_amount")

    l10n_mx_edi_date = fields.Date(
        "Date (att)", tracking=True, help="Date on the CFDI attached [If 1 if several we will need to split them]"
    )
    l10n_mx_edi_subtotal = fields.Float("Amount Subtotal")
    l10n_mx_edi_tax = fields.Float("Tax", tracking=True)
    l10n_mx_edi_discount = fields.Float("Discount", tracking=True)
    l10n_mx_edi_withhold = fields.Float("Withhold", tracking=True)
    ...
    l10n_mx_edi_analysis_html = fields.Html("Analysis HTML", compute="_compute_analysis_html", tracking=True)
    ...
    l10n_mx_edi_functional_details_html = fields.Html("Functional", compute="_compute_functional_details_html")
    l10n_mx_edi_accountant = fields.Many2one(
        "res.users",
        string="Accountant",
    ...
    def _compute_is_editable(self):
        result = super()._compute_is_editable()
        for expense in self.filtered(lambda e: e.state == "downloaded"):
    ...
    def get_expense_errors(self):
        ...
        if len(self.mapped("employee_id")) != 1:
            errors.append(_("You cannot report expenses for different employees in the same report."))
        if any(expense.state != "draft" for expense in self):
            errors.append(
                _(
                    "You can not report a line in any state different"
                    " from draft if it is in **Downloaded** state wait for the automatic check."
                )
            )
        if any(expense.state != "draft" for expense in self):
            errors.append(
                _(
                    "You can not report a line in any state different"
                    " from draft if it is in **Just Downloaded** state wait for the automatic check."
                )
            )
        if not all(expense.l10n_mx_edi_functionally_approved for expense in self):
            errors.append(
                _(
                    "This expense is not functionally approved, see "
                    "errors or ask your supervisor to force the functional exception"
                )
            )
        if not all(expense.l10n_mx_edi_fiscally_approved for expense in self):
            errors.append(
                _(
                    "This expense looks as do not comply with the "
                    "fiscal validation, please load a valid invoice "
                    "or ask you supervisor to approve this exception, sending a message."
                )
            )
        if any(expense.state != "draft" for expense in self):
            errors.append(
        ...
        if any(not expense.employee_id.journal_id for expense in self if expense.payment_mode == "own_account"):
            errors.append(
                _("You can not report a line paid by the employee and that they do not have a journal configured.")
            )
        if len(set(self.mapped("payment_mode"))) > 1:
            errors.append(_("You can not report lines with different payment mode."))
        ...
        label = self.env.ref("l10n_mx_edi_hr_expense.tag_force_invoice_generation", False)
        if any(exp for exp in self if exp.l10n_mx_edi_is_to_check and label not in exp.partner_id.category_id):
            errors.append(
                _(
                    "This expense is marked to be checked but the "
                    "supplier does not have the label that forces the invoice generation."
                )
            )
        return errors
    ...
    def action_submit_expenses(self):
        ...
        if errors:
            raise UserError("\n".join(errors))
        todo = (
            self.filtered(lambda x: x.payment_mode == "own_account")
            or self.filtered(lambda x: x.payment_mode == "company_account")
            or self.filtered(lambda x: x.payment_mode == "petty_account")
        )
        ...
        return {
            ...
            "context": {
                ...
                "default_petty_journal_id": employee.journal_ids.id
                if len(employee.journal_ids) == 1 and todo[0].payment_mode == "petty_account"
                else False,
            },
        }
    ...
    def _compute_functional_details_html(self):
        for expense in self:
            expense.l10n_mx_edi_functional_details_html = self.json2qweb(expense.l10n_mx_edi_functional_details)
    ...
    def json2qweb(self, json_input=None):
        ...
        template = self.env.ref("l10n_mx_edi_hr_expense.expense_checks_content")
    ...
    def _compute_analysis_html(self):
        for expense in self:
            expense.l10n_mx_edi_analysis_html = json2html.convert(expense.l10n_mx_edi_analysis)

    l10n_mx_edi_rfc = fields.Text("Emitter")
    l10n_mx_edi_received_rfc = fields.Text("Received By")
    state = fields.Selection(
        selection_add=[("downloaded", "Just Downloaded")],
        default="downloaded",
        readonly=False,
    ...
    l10n_mx_edi_sat_status = fields.Selection(
        selection=[
            ("none", "State not defined(answer on sat not possible to manage)"),
            ...
            ("more_than_one", "Please split in several lines there is more than one invoice to check with SAT"),
        ],
        string="SAT status",
    ...
    l10n_mx_count_cfdi = fields.Integer("Count CFDI's", tracking=True)
    l10n_mx_edi_invoice_id = fields.Many2one(
        "account.move", "Invoice", help="Invoice created with this expense", readonly=True, copy=False, index=True
    )
    l10n_mx_edi_document_type = fields.Selection(
        [("in_invoice", "Vendor Bill"), ("in_refund", "Vendor Credit Note")],
        "Document Type",
        help="Save the document type in the CFDI.",
    )
    ...
    def create_partner_from_cfdi(self):
        ...
        for expense in self.filtered("l10n_mx_edi_rfc"):
            ...
            if not partner:
                domain.pop()
                partner = partner.sudo().search(domain, limit=1)
            global_partner = self.env["ir.config_parameter"].sudo().get_param("l10n_mx_edi_hr_expense.global_partner")
            if not partner and global_partner:
                partner = self.env["res.partner"].browse(int(global_partner))
            if not partner:
    ...
    def _compute_partner_id(self):
        ...
        for expense in self.filtered("l10n_mx_edi_rfc"):
            ...
            expense_partner = partner.search(
                [("vat", "=", expense.l10n_mx_edi_rfc), ("is_company", "=", True), ("vat", "!=", False)], limit=1
            )
            if not expense_partner:
                expense_partner = partner.search(
                    [("vat", "=", expense.l10n_mx_edi_rfc), ("vat", "!=", False)], limit=1
                )
    ...
    def _compute_amount(self):
        ...
        for expense in self.filtered("l10n_mx_edi_analysis"):
            total = json.loads(expense.l10n_mx_edi_analysis).get("invoices")[0].get("total")
    ...
    def _compute_state(self):
        for expense in self:
            if not expense.sheet_id:
                ...
                expense.sheet_id.state in ("done", "post")
                and expense.l10n_mx_edi_invoice_id
                and expense.l10n_mx_edi_invoice_id.payment_state not in ("in_payment", "paid")
            ):
                expense.state = "approved"
            elif expense.sheet_id.state == "done" or expense.l10n_mx_edi_invoice_id.payment_state in (
                "in_payment",
                "paid",
            ):
                expense.state = "done"
        ...
        with Environment.manage():
            with registry(self.env.cr.dbname).cursor() as _cr:
                ...
                employee = (
                    ...
                    .sudo()
                    .search([("user_id", "=", SUPERUSER_ID)], limit=1)
                )
                if not employee:
                ...
                # active..
                employee.write({"active": True, "work_email": "__system__@__system__.com"})
                _cr.commit()
                return employee.id
    ...
    def ack(self):
        ...
        **context**:
        ...
        """
        avoid_email = self.env["ir.config_parameter"].sudo().get_param("l10n_mx_edi_hr_expense_avoid_email", "")
        states = ["downloaded", "draft", "reported", "approved", "paid", "refused"]
        mode = self.env.context.get("mode")
        user = self.env.context.get("user")
        template = self.env.ref("l10n_mx_edi_hr_expense.mail_template_expense_%s" % mode, raise_if_not_found=False)
        if avoid_email or mode not in states or not template:
            return
        ...
        for expense in self:
            ...
            if not user:
                _logger.error(
                    "Impossible to send any ACK from expenses if employee incorrectly set.",
                )
            ...
            _logger.info(
                "Expenses email was sent for expense <%s> with name <%s> with id: %s", expense.id, expense.name, mail
            )

    def email_split(self, msg):
        """Copied from the technique used in tasks to set the
        ...
        email_list = tools.email_split(
            (msg.get("to") or "") + "," + (msg.get("cc") or "") + "," + (msg.get("email_from") or "")
        )
        # check left-part is not already an alias
        aliases = self.env.ref("hr_expense.mail_alias_expense").mapped("display_name")
    ...
    def message_new(self, msg, custom_values=None):
        ...
        domain = ["|", ("work_email", "ilike", email_address), ("user_id.email", "ilike", email_address)]
        ...
        if product_code is None:
            ...
            products = (
                self.env["product.product"].search([("default_code", "ilike", product_code.group(1))])
                or default_product
            )
        ...
        expense = super().message_new(msg, custom_values)
    ...
    def l10n_mx_edi_update_sat_status(self):
        ...
        for rec in self:
            ...
            total = float_repr(rec.total_amount, precision_digits=rec.currency_id.decimal_places)
            uuid = rec.l10n_mx_edi_uuid
            status = edi_format._l10n_mx_edi_get_sat_status(supplier_rfc, customer_rfc, total, uuid)
            rec.l10n_mx_edi_sat_status = CFDI_SAT_QR_STATE.get(status)
    ...
    def check_fiscal_status(self):
        ...
        for expense in expenses:
            ...
            for att in invoices.sudo().filtered(lambda inv: inv.res_id == expense.id):
                datas = base64.b64decode(att.datas).replace(b"xmlns:schemaLocation", b"xsi:schemaLocation")
                invoice = objectify.fromstring(datas)
                currency = currency.search([("name", "=", invoice.get("Moneda", ""))], limit=1)
                cfdi_related = []
                relation_type = ""
                if hasattr(invoice, "CfdiRelacionados"):
                    relation_type = invoice.CfdiRelacionados.get("TipoRelacion")
                ...
                if hasattr(invoice, "Impuestos"):
                    ...
                    total_ieps = 0
                    for tax in taxes.Traslados.Traslado if hasattr(taxes, "Traslados") else []:
                        if tax.get("Impuesto") != "003":
                            continue
                        if float(tax.get("TasaOCuota")) == 0.265:
                            continue
                        total_ieps += float(tax.get("Importe"))
                    data.update(
                        {
                            "tax": float(invoice.Impuestos.get("TotalImpuestosTrasladados", "0.0")),
                            "withhold": float(invoice.Impuestos.get("TotalImpuestosRetenidos", "0.0")),
            ...
            if not res:
                _logger.info("Nothing fiscally valid on expense: %s", expense.id)
                return {}
            ...
            references = ", ".join(["%s/%s" % (i["serie"], i["number"]) for i in res])
            expense.update(
                {
                    ...
                    "l10n_mx_edi_document_type": dict_document_type.get(document_type, False),
                }
            )
            if not expense.company_id:
                company = self.env["res.company"].search([("vat", "=", invoice.Receptor.get("Rfc", " "))], limit=1)
    ...
    def _get_taxes_line(self, line):
        ...
        for tax in taxes.Retenciones.Retencion if hasattr(taxes, "Retenciones") else []:
            if float_is_zero(float(tax.get("TasaOCuota", "")), precision_digits=6):
                continue
            taxes_line.append(
                {
                    "type": "ret",
    ...
    def functional3(self):
        ...
        return not bool(
            self.sudo().search(
                [("l10n_mx_edi_uuid", "in", uuids), ("id", "not in", self.ids), ("l10n_mx_edi_uuid", "!=", False)]
            )
        )
    ...
    def functional11(self):
        payment = json.loads(self.l10n_mx_edi_analysis).get("invoices")[0].get("payment")
        if payment != "PPD" and self.payment_mode == "company_account":
            return False
        return True

    def functional12(self):
        payment = json.loads(self.l10n_mx_edi_analysis).get("invoices")[0].get("payment_way")
    ...
    def functional14(self):
        ...
        for line in invoice[0].get("lines", []):
            for tax in line["taxes"]:
                if tax["tax"] != "003" and tax['rate'] != 0.265 and not self.l10n_mx_edi_get_tax(tax):
                    return False
        return True

    def functional15(self):
        """Check that the employee has a journal"""
        if not self.employee_id.journal_id and self.payment_mode in ["own_account", "petty_account"]:
            return False
        return True
    ...
    def functional17(self):
        ...
        for cfdi in data["cfdi_related"]:
            ...
            ).filtered(lambda inv: inv.l10n_mx_edi_cfdi_uuid == cfdi) and not self.search(
                [("l10n_mx_edi_uuid", "=", cfdi), ("partner_id", "=", self.partner_id.id)], limit=1
            ):
                return False
        return True

    def functional18(self):
        """Checking that if the CFDI is a refund, the node CfdiRelacionado is found"""
        data = json.loads(self.l10n_mx_edi_analysis).get("invoices")[0]
        document_type = self.l10n_mx_edi_document_type
    ...
    def functional20(self):
        """If the CFDI have local taxes complement, verify that the taxes are found and the account is configured."""
        data = json.loads(self.l10n_mx_edi_analysis).get("invoices")[0]
        if not data.get("local_taxes"):
            return True
        ...
        for tax in data["local_taxes"]:
            if tax["tax"].upper() in self._get_taxes_to_omit():
                continue
            local = self.l10n_mx_edi_get_local_tax(tax)
            if local and not local.cash_basis_transition_account_id:
                return False
        return True
    ...
    def _render_email_check(self, result):
        ...
        :params: result Json with all errors that not passed the check
                        ...
                        'fail': {code: {'title': 'Subject',
                                        'message', 'Message'}
        """
        ok = {k: self._remove_method(v) for (k, v) in self.functional_errors().items() if k not in result}
    ...
    def check_functional(self):
        ...
        if not self.l10n_mx_edi_analysis or not json.loads(self.l10n_mx_edi_analysis).get("invoices"):
            self.sudo().message_post(
                body=_(
                    "This expense is not possible to verify functionally "
                    "because there is not CFDI associated yet, add the xml "
                    "or ask for extra permissions to propose as an expense."
                )
            )
            return
        ...
        self.write(
            {
                "l10n_mx_edi_functional": "fail" if result else "ok",
    ...
    def functional_errors(self):
        return {
            1: {
                ...
                "message_ok": _("The RFC used on this CFDI is the one on this company"),
            ...
            2: {
                ...
                "title_ok": _("New UUID"),
                "message": _("The XML UUID belongs to other invoice already loaded on the system."),
                "message_ok": _("No other invoice with this uuid has been declared in the system"),
                "check": self.functional2,
            },
            3: {
                "title": _("UUID Duplicated on expenses"),
                "title_ok": _("UUID Without declaration"),
                "message": _("The XML UUID belongs to other expense already loaded on the system."),
                "message_ok": _("Neither you or somebody else try to declare this UUID as expenses before."),
            ...
            4: {
                ...
                "message": _(
                    "The administrative team will need to check if "
                    "invoices for this vendor are valid, and if we "
                    "need to pay something to them this will need "
                    "further communication to have the payment information."
                ),
                "message_ok": _("We have old invoices from this vendor, stay in touch if we need something extra."),
                "check": self.functional4,
            },
            5: {
                "title": _("Currency Disabled"),
                "title_ok": _("Currency Active"),
                "message": _("The currency in the XML was not found or is disabled"),
                "message_ok": _("The currency declared in the CFDI is valid and is configured properly"),
            ...
            6: {
                ...
                "message": _(
                    "The date on the CFDI is not on this period, this will need and extra approval of your expense"
                ),
                "message_ok": _(
                    "It is inside the safety period (current period or max 5 days at the beginning of the next)"
                ),
            ...
            7: {
                ...
                "message_ok": _("We reviewed if the zip code of the receiver company is the same than this company"),
            ...
            8: {
                ...
                "message": _(
                    "This expense is not deductible, it looks like an expense without valid CFDI in the invoice"
                ),
                "message_ok": _("I checked if there is a valid xml in the expense attached"),
            ...
            9: {
                ...
                "message": _(
                    "This invoice reference was loaded for this "
                    "partner invoice reference belongs to other "
                    "invoice of the same partner."
                ),
                "message_ok": _("The reference (folio/serie) is first time we see it in this system"),
            ...
            10: {
                ...
                "message": _(
                    "It looks like you tried to create an expense "
            ...
            11: {
                ...
                "title_ok": _("Payment Method is PPD and will be paid by us or PUE and was paid by the employee"),
                "message": _(
                    "It looks like this expense must be paid by the "
                    ...
                    "again to check functionally (clicking on the button)"
                ),
            ...
            13: {
                ...
                "message": _("It looks like the CFDI attached is not registered in the SAT system or is cancelled."),
            ...
            14: {
                ...
                "message": _("The taxes in the XML was not found or are disabled"),
                "message_ok": _("The taxes declared in the CFDI are valid and are configured properly"),
            ...
            15: {
                ...
                "message": _(
                    "The journal in the employee is required for expenses paid for the employee or with petty cash."
                ),
                "message_ok": _(
                    "This expense was paid for the employee or "
                    "with petty cash and the journal is correctly configured."
                ),
            ...
            16: {
                ...
                "message": _(
                    "It looks like you tried to create an expense "
                    "for a refund, and in that case must be paid by the company."
                ),
                "message_ok": _(
                    "It looks like you tried to create an expense for a refund, and is paid by the company."
                ),
                "check": self.functional16,
            },
            17: {
                "title": _("The CFDI is a refund, and the CFDI related is not found"),
                "title_ok": _("The CFDI is a refund, and the CFDI related is found!"),
                "message": _(
                    "It looks like you tried to create an expense "
                    "for a refund, but the CFDI related is not found."
                    " First create the invoice and after the refund."
                ),
                "message_ok": _("The CFDI is a refund and the CFDI related is found in the instance."),
                "check": self.functional17,
            },
            18: {
                "title": _("The CFDI is a refund, and the node 'CfdiRelacionados' is not found"),
                "title_ok": _("The CFDI is a refund, and the node 'CfdiRelacionados' is found"),
                "message": _(
                    "It looks like you tried to create an expense "
                    "for a refund, but the node 'CfdiRelacionados' is not found."
                ),
                "message_ok": _(
                    "It looks like you tried to create an expense "
            ...
            19: {
                "title": _("The CFDI have Local Taxes and that are not found"),
                "title_ok": _("The CFDI have Local Taxes and that are found"),
                "message": _(
                    "It looks like the CFDI have Local Taxes complement and some of that taxes are not found."
                ),
                "message_ok": _("It looks like the CFDI have Local Taxes complement and that taxes are found."),
                "check": self.functional19,
            },
            20: {
                "title": _("The CFDI have Local Taxes and that are found but is missing the account in the tax"),
                "title_ok": _("The CFDI have Local Taxes, the taxes are found and are properly configured"),
                "message": _(
                    "It looks like the CFDI have Local Taxes "
    ...
    def force_functional(self):
        for exp in self:
            if not self.env.user.has_group("l10n_mx_edi_hr_expense.force_expense"):
                raise UserError(
                    _(
                        "You can not force to approve an expense "
                        "by any mean if you do not belong to the proper group"
                    )
                )
    ...
    def action_open_invoices(self):
        ...
        return {
            ...
            "view_id": False,
            "type": "ir.actions.act_window",
            "domain": [("id", "in", self.mapped("l10n_mx_edi_invoice_id").ids)],
            "context": {
                "move_type": "in_invoice",
            },
        }
    ...
    def l10n_mx_edi_create_expense_invoice(self):
        ...
        expenses = self.filtered(
            lambda e: e.l10n_mx_edi_functionally_approved
            and e.l10n_mx_edi_fiscally_approved
            and e.l10n_mx_edi_analysis
        )
        label_draft = self.env.ref("l10n_mx_edi_hr_expense.tag_invoice_draft")
        expenses_forced = self._get_expenses_force_invoice_generation()
        expenses_forced.l10n_mx_edi_create_invoice_wh_cfdi()
        to_omit = (expenses + expenses_forced).filtered(
            lambda exp: exp.l10n_mx_edi_invoice_id.state and exp.l10n_mx_edi_invoice_id.state != "cancel"
        )
        for exp in to_omit:
            exp.message_post(body=_("This expense already have an invoice related."))
        for exp in expenses - to_omit:
            ...
            invoice = (
                self.env["account.move"].sudo().with_context(context).create(data)
            ...
            exp.l10n_mx_edi_invoice_id = invoice
            ...
            if exp.payment_mode == "company_account" or (exp.payment_mode == "petty_account" and not petty_cash):
                continue
            ctx = {"active_model": "account.move", "active_ids": invoice.ids}
            journal = exp.employee_id.journal_id if exp.payment_mode == "own_account" else petty_cash
    ...
    def _get_expenses_force_invoice_generation(self):
        """Return expenses that must force the invoice generation."""
        label = self.env.ref("l10n_mx_edi_hr_expense.tag_force_invoice_generation", False)
        return self.filtered(
            lambda e: e.l10n_mx_edi_functionally_approved
            and e.l10n_mx_edi_fiscally_approved
            and not e.l10n_mx_edi_analysis
            and label in e.partner_id.category_id
        )
    ...
    def create_extra_line_local_taxes(self, tax):
        self.ensure_one()
        ieps = (
            self.env.ref("l10n_mx_edi_hr_expense.product_product_ieps", False)
            if "IEPS" in tax["tax"].upper()
            else False
        )
        account = ieps.product_tmpl_id.get_product_accounts()["expense"].id if ieps else self.account_id.id
        return {
            "product_id": ieps.id if ieps else self.product_id.id,
            "account_id": account,
            "name": tax["tax"].upper(),
            "quantity": 1,
            "product_uom_id": ieps.uom_id.id if ieps else self.product_uom_id.id,
            "price_unit": tax["amount"],
            "analytic_account_id": self.analytic_account_id.id,
            "analytic_tag_ids": [(6, 0, self.analytic_tag_ids.ids)],
        }

    def l10n_mx_edi_get_invoice_data_get_lines(self, inv):
        invoice = self.env["account.move"]
        ...
        document_type = self.l10n_mx_edi_document_type
        journal_ctx = {
            "default_journal_id": self.sheet_id.journal_id.id,
            "default_move_type": document_type,
            "default_currency_id": self.currency_id.id,
            "default_company_id": self.company_id.id,
        }
        journal = invoice.with_context(**journal_ctx)._get_default_journal()
        account = self.account_id.id or journal.default_account_id.id
        invoice_line_ids = []
        tax_global = self._get_tax_global()
        label = self.env.ref("l10n_mx_edi_hr_expense.tag_force_invoice_total")
        for line in inv.get("lines", []):
            no_id = line["no_id"]
            restrict_product_search = (
                self.env["ir.config_parameter"].sudo().get_param("l10n_mx_edi_hr_expense.restrict_product_search")
            )
            res = False
            if restrict_product_search:
                res = {"id": self.product_id.id}
            if not res:
                self._cr.execute(
                    """SELECT pp.id
            ...
            account = prod.browse(product).product_tmpl_id.get_product_accounts()["expense"].id if res else account
            amount = line["amount"]
            discount = (line["discount"] / amount) * 100 if line["discount"] and amount else 0.0

            code_sat = sat_code.search([("code", "=", line["uom_code"])], limit=1)
            uom_id = prod.browse(product).uom_id
            if not uom_id or uom_id.unspsc_code_id.code != line["uom_code"]:
                uom_id = uom.search([("unspsc_code_id", "=", code_sat.id)], limit=1)

            line_taxes = []
            taxes = tax_global or line["taxes"]
            for tax in taxes:
                if tax["tax"] == "003" and tax["rate"] != 0.265:
                    continue
            ...
            if (
                label in self.partner_id.category_id
                and float_compare(total_line, qty * price, precision_digits=2) != 0
            ):
                price = total_line / qty
            if line["sat_code"] in self._get_fuel_codes() and len(line["taxes"]) == 1:
                qty = 1.0
    ...
    def l10n_mx_edi_get_invoice_data_get_taxes(self, inv):
        invoice_line_ids = []
        invoice = self.env["account.move"]
        for tax in inv.get("local_taxes"):
            if tax["tax"].upper() in self._get_taxes_to_omit():
                invoice_line_ids.append((0, 0, self.create_extra_line_local_taxes(tax)))
                continue
            invoice_line_ids.append(
                (
                    0,
                    0,
                    {
                        ...
                        "name": tax["tax"],
                        "price_unit": tax["amount"] * (-1 if tax["type"] == "ret" else 1),
                        "quantity": 1,
                        "move_id": invoice.id,
                    },
                )
    ...
    def l10n_mx_edi_get_invoice_data_get_ieps(self, inv, account):
        invoice_line_ids = []
        if inv.get("total_ieps"):
            product_ieps = self.env.ref("l10n_mx_edi_hr_expense.product_product_ieps", False)
            account = product_ieps.product_tmpl_id.get_product_accounts()["expense"].id if product_ieps else account
            invoice_line_ids.append(
                (
                    0,
                    0,
                    {
                        ...
                        "quantity": 1,
                        "product_uom_id": product_ieps.uom_id.id if product_ieps else self.product_uom_id.id,
    ...
    def l10n_mx_edi_get_invoice_data_get_payment_term(self, inv):
        acc_pay_term = self.env["account.payment.term"]
        pay_term_immediate = self.env.ref("account.account_payment_term_immediate", False) or acc_pay_term
        acc_pay_term = acc_pay_term.search([("name", "=", inv["payment_conditions"])], limit=1)
        if not acc_pay_term:
            acc_pay_term = (
                self.partner_id.property_supplier_payment_term_id
                if inv["payment"] in ("PPD", False)
                else pay_term_immediate
            )
        return acc_pay_term

    def l10n_mx_edi_get_invoice_data_get_cfdi_origin(self, inv):
        if inv.get("relation_type"):
            return "%s|%s" % (inv["relation_type"], ",".join(inv["cfdi_related"]))
        return ""
    ...
    def l10n_mx_edi_get_invoice_data(self, inv):
        ...
        invoice_line_ids = self.l10n_mx_edi_get_invoice_data_get_lines(inv)
        invoice_line_ids += self.l10n_mx_edi_get_invoice_data_get_taxes(inv)
        invoice_line_ids += self.l10n_mx_edi_get_invoice_data_get_ieps(inv, account)

        payment_method = self.env["l10n_mx_edi.payment.method"].search([("code", "=", inv["payment_way"])], limit=1)
        acc_pay_term = self.l10n_mx_edi_get_invoice_data_get_payment_term(inv)
        cfdi_origin = self.l10n_mx_edi_get_invoice_data_get_cfdi_origin(inv)
        return {
            "partner_id": self.partner_id.id,
            ...
            "invoice_date": self.l10n_mx_edi_date,
            "date": self.date,
    ...
    def l10n_mx_edi_get_tax(self, tax, product_id=False):
        ...
        if tax["type"] == "ret":
            rate = rate * -1
        domain = [
            ("invoice_repartition_line_ids.tag_ids", "in", tag_codes.get(tax["tax"])),
            ("type_tax_use", "=", "purchase"),
            ("amount", "=", rate),
        ]
        if tax["type"] == "ret" and -10.67 <= rate <= -10.66:
            domain[-1] = "|"
    ...
    def l10n_mx_edi_get_tax_group(self, taxes):
        ...
        tag_taxes = (
            tax.browse(taxes).sudo().filtered(lambda tax: tag_tax in tax.invoice_repartition_line_ids.tag_ids.ids)
        )
        ...
        tax5 = tax.search([("description", "=", tax16.description), ("amount", "=", 5.3333)])
        if not tax5 or not grouped_taxes:
            return taxes
        taxes_copy = taxes[:]
        taxes.remove(tax16.id)
        taxes.append(tax5.id)
        tag_taxes = tax.browse(taxes).filtered(lambda tax: tag_tax in tax.invoice_repartition_line_ids.tag_ids.ids)
        for group in grouped_taxes:
            if set(tag_taxes.ids).issubset(group.children_tax_ids.ids):
                return (
                    group
                    + tax.browse(taxes).filtered(
                        lambda tax: tag_tax not in tax.invoice_repartition_line_ids.tag_ids.ids
                    )
                ).ids
        # Here change the tax 16.0 for 12 for "Fletes"
        tax12 = tax.search([("description", "=", tax16.description), ("amount", "=", 12.0)])
    ...
    def l10n_mx_edi_get_cfdi(self):
        ...
        for att in attachments:
            # Fix XML from SAT
            datas = base64.b64decode(att.datas).replace(b"xmlns:schemaLocation", b"xsi:schemaLocation")
            att.datas = base64.b64encode(datas)
    ...
    def _onchange_l10n_mx_employee_id(self):
        self.payment_mode = self.employee_id.l10n_mx_edi_payment_mode or self.payment_mode

    def action_move_create(self):
        label = self.env.ref("l10n_mx_edi_hr_expense.tag_force_invoice_generation", False)
        expenses = self.filtered(
            lambda exp: not exp.l10n_mx_edi_analysis
            and label not in exp.partner_id.category_id
            and not exp.l10n_mx_edi_move_line_id
        )
        res = super(HrExpense, expenses).action_move_create()
        lines = self.env["account.move.line"]
        for expense in expenses:
            line = lines.search([("expense_id", "=", expense.id), ("product_id", "=", expense.product_id.id)])
    ...
    def l10n_mx_edi_reclassify_journal_entries(self):
        ...
        compose_form = self.env.ref("l10n_mx_edi_hr_expense.view_l10n_mx_edi_reclassify_journal_entries", False)
        return {
    ...
    def get_pdf_expenses(self):
        ...
        return self.env["ir.attachment"].search(
            [("res_id", "=", self.id), ("res_model", "=", self._name), ("name", "ilike", ".pdf")]
        )
    ...
    def l10n_mx_edi_get_local_tax(self, tax):
        ...
        if tax["type"] == "ret":
            rate = rate * -1
        domain = [
            ("invoice_repartition_line_ids.tag_ids", "in", local.id),
            ("type_tax_use", "=", "purchase"),
            ("amount", "=", rate),
        ]
        if tax["type"] == "ret" and -10.67 <= rate <= -10.66:
    ...
    def split_expense(self):
        ...
        for att in invoice_atts[:-1]:
            count += 1
            expense = self.copy({"name": "%s - %s" % (self.name, count)})
            expenses += expense
            att.write({"res_id": expense.id})
            att_name = splitext(att.name)[0]
            alt_atts = not_invoice_atts.filtered(lambda _att: (splitext(_att.name)[0] == att_name))
            alt_atts.write({"res_id": expense.id})
            not_invoice_atts -= alt_atts
            expense.message_post_with_view(
                "l10n_mx_edi_hr_expense.hr_expense_splited_from", values={"self": expense, "origin": self}
            )
        expenses -= self
        self.message_post_with_view(
            "l10n_mx_edi_hr_expense.hr_expense_splited_to", values={"self": self, "dest": expenses}
        )
    ...
    def l10n_mx_edi_create_invoice_wh_cfdi(self):
        ...
        for expense in self:
            ...
            journal = invoice.with_context(default_move_type=inv_type)._get_default_journal()
            product = expense.product_id
            account = product.product_tmpl_id.get_product_accounts()["expense"].id
            invoice_line_ids = [
                (
                    0,
                    0,
            ...
            data = {
                ...
                "invoice_date": expense.l10n_mx_edi_date or expense.date,
                "date": expense.date,
            ...
            if context.get("default_name"):
                context.pop("default_name")
            inv = invoice.sudo().with_context(context).create(data)  # pylint: disable=context-overridden
            inv.with_context(check_move_validity=False)._onchange_invoice_date()
            expense.l10n_mx_edi_invoice_id = inv
            inv.l10n_mx_edi_expense_id = expense

    def l10n_mx_edi_revert_expense(self):
        if not self.user_has_groups("l10n_mx_edi_hr_expense.allow_to_revert_expenses"):
            raise UserError(_('Only the users in the group "Allow to Revert Expenses" could execute this action.'))
        move_ids = self.env["account.move"]
        for record in self.filtered("sheet_id"):
            invoice = record.l10n_mx_edi_invoice_id
            if invoice and invoice.state != "cancel":
                invoice._get_reconciled_payments().action_cancel()
            ...
            if not invoice and record.l10n_mx_edi_move_line_id:
                move = record.l10n_mx_edi_move_id
                move.button_cancel()
                move.line_ids.filtered(lambda aml: aml.expense_id == record).unlink()
                if not move.line_ids:
                    move_ids |= move
                    record.sheet_id.account_move_id = False
            record.state = "approved"
            record.sheet_id.state = "approve"
        move_ids.exists().with_context(force_delete=True).unlink()

    def _get_expense_account_destination(self):
        self.ensure_one()
        res = super()._get_expense_account_destination()
        if self.payment_mode not in ("petty_account", "own_account"):
            return res
        sheet = self.sheet_id
        journal = sheet.petty_journal_id if self.payment_mode == "petty_account" else sheet.employee_id.journal_id
        if not journal:
            return res
    ...
    def _compute_is_ref_editable(self):
        ...
        res = super()._compute_is_ref_editable()
