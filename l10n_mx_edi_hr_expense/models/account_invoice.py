...
class AccountMove(models.Model):
    ...
    l10n_mx_edi_expense_id = fields.Many2one(
        "hr.expense", "Expense", help="Stores the expense related with this invoice"
    )
    l10n_mx_edi_expense_sheet_id = fields.Many2one(
        "hr.expense.sheet",
        string="Expense Sheet",
        related="l10n_mx_edi_expense_id.sheet_id",
        store=True,
    ...
    def action_post(self):
        ...
        label = self.env.ref("l10n_mx_edi_hr_expense.tag_omit_invoice_amount_check")
        partners = self.mapped("partner_id").filtered(lambda par: label not in par.category_id)
        invoices = self.filtered(lambda inv: inv.l10n_mx_edi_cfdi_amount and inv.partner_id in partners)
        for invoice in invoices.filtered(lambda inv: inv.move_type in ("in_invoice", "in_refund")):
        ...
        for invoice in invoices.filtered(lambda inv: inv.move_type in ("out_invoice", "out_refund")):
            diff = invoice.journal_id.l10n_mx_edi_amount_authorized_diff
            if not abs(invoice.amount_total - invoice.l10n_mx_edi_cfdi_amount) > diff:  # noqa
                continue
            currency = invoice.currency_id
    ...
    def action_view_expense(self):
        self.ensure_one()
        expense = self.env["hr.expense"].search([("l10n_mx_edi_invoice_id", "=", self.id)], limit=1)
        if not expense:
