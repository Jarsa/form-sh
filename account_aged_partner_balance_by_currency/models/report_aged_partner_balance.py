...
class AccountAgedPartnerBalance(models.AbstractModel):
    ...
    # Copy the current method in Odoo and change it to comply with this module
    # functionality. It was renamed to avoid errors with other reports.
    # pylint: disable=too-complex
    # pylint: disable=sql-injection
    # pylint: disable=comprehension-escape
    # pylint: disable=range-builtin-not-iterating
    def _get_partner_move_lines_2(
            self, account_type, date_from, target_move, period_length,
            currency_id=False):
        ...
        # In case of a period_length of 30 days as of 2019-02-08, we want the
        # following periods:
        # Name       Stop         Start
        ...
        if target_move == 'posted':
            move_state = ['posted']
        arg_list = (
            tuple(move_state), tuple(account_type), date_from, date_from,)
        ...
        if isinstance(currency_id, int):
            report_currency = self.env['res.currency'].browse(currency_id)
            currency_clause = 'AND l.currency_id = %s' % currency_id
            if user_currency.id == currency_id:
                currency_clause = 'AND l.currency_id IS NULL'
        arg_list += (date_from, tuple(company_ids))
        query = '''
            SELECT DISTINCT l.partner_id, UPPER(res_partner.name)
            FROM account_move_line AS l left join res_partner on
            l.partner_id = res_partner.id, account_account, account_move am
            WHERE (l.account_id = account_account.id)
                ...
                AND (
                        ...
                        OR l.id IN(
                            SELECT credit_move_id FROM
                            account_partial_reconcile where max_date > %s
                            UNION ALL
                            SELECT debit_move_id FROM account_partial_reconcile
                            where max_date > %s
                        )
                    )
                    ''' + partner_clause + currency_clause + '''
                AND (l.date <= %s)
        ...
        partner_ids = [
            partner['partner_id'] for partner in partners if partner[
                'partner_id']]
        lines = dict(
            ...
            for line in self.env['account.move.line'].browse(
                ...
                for partial_line in line.matched_debit_ids:
                    if partial_line.max_date <= date_from:
                        line_amount += (
                            partial_line.company_id.currency_id._convert(
        ...
        query = '''SELECT l.id
                ...
                WHERE (l.account_id = account_account.id)
                    ...
                    AND ((l.partner_id IN %s) OR (l.partner_id IS NULL))
                AND (l.date <= %s)
                AND l.company_id IN %s''' + currency_clause + '''
                ORDER BY COALESCE(l.date_maturity, l.date)'''
        cr.execute(
            query, (
        ...
        for line in self.env['account.move.line'].browse(aml_ids):
            ...
            if isinstance(
                    currency_id, int) and report_currency != user_currency:
                line_amount = line.amount_currency
            else:
                line_amount = (
                    line.company_id.currency_id._convert(
                        line.balance, user_currency, user_company, date_from))
            if user_currency.is_zero(line_amount):
                continue
            for partial_line in line.matched_debit_ids:
                if partial_line.max_date <= date_from:
                    if isinstance(currency_id, int):
                        line_amount += (
                            partial_line.company_id.currency_id._convert(
                                partial_line.amount, report_currency,
                                user_company, date_from))
                    else:
                        line_amount += (
                            partial_line.company_id.currency_id._convert(
        ...
        for partner in partners:
            ...
            if partner['partner_id']:
                # browse the partner name and trust field in sudo, as we may
                # not have full access to the record (but we still have to see
                # it in the report)
                browsed_partner = self.env['res.partner'].sudo().browse(
                    partner['partner_id'])
                values['name'] = (
                    browsed_partner.name and
