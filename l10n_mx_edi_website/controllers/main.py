        ...
        try:
            with request.env.cr.savepoint():
                values = (
                    values
                    if values.get("error")
                    else request.env["pos.order"].sudo().update_partner(ticket_number, vat, email)
                )
        except Exception as msg_error:
            _logger.error(msg_error)
    ...
    def _download_attached_file(self, ticket, ftype="pdf"):
        ...
        with api.Environment.manage():
            ...
            if inv and inv.edi_state == "sent" and not attachment and ftype == "pdf":
                ...
                report = env["ir.actions.report"]._get_report_from_name(report_info["report_name"])
            ...
            if attachment:
                status, headers, content = request.env["ir.http"].binary_content(id=attachment.id, download=True)
