...
class PosOrder(models.Model):
    ...
    def _get_invoice_from_close_session(self):
        ...
        :rtype: account.move()
        """
        config = self.env.ref("l10n_mx_edi_website.pos_config_download_electronic_invoice")
        config.active = True
        ...
        session = session_obj.search(
            ...
            limit=1,
        ) or session_obj.create({"user_id": self._uid, "config_id": config.id})
        partner_id = self.partner_id.id
        ticket_number = self.l10n_mx_edi_ticket_number
        self.write({"partner_id": False, "l10n_mx_edi_ticket_number": None})
        session.action_pos_session_open()
        returns = self.refund()
        returns = self.browse(returns.get("res_id"))
        # To be sure that the record is in my session
        returns.write(
            {"session_id": session.id, "note": _("Created to generate a new order and generate the invoice")}
        )
