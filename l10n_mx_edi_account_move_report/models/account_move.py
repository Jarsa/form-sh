...
class AccountMove(models.Model):
    ...
    def l10n_mx_edi_get_uuid(self):
        ...
        # Exchange difference
        full_reconcile = self.env["account.full.reconcile"].search([("exchange_move_id", "in", all_move.ids)])
        ...
        if invoices:
            cfdi["type"] = {
                ...
                "out_refund": "Egreso",
                "in_invoice": "Egreso",
            }.get(invoices[0].move_type)
        # Payment in PUE
        payment_without_cfdi = payments - cfdi["payments"]
        cfdi["invoices"] |= payment_without_cfdi.reconciled_invoice_ids.filtered(lambda i: i.l10n_mx_edi_cfdi_uuid)
        return cfdi
