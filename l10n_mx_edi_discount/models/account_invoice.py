...
class AccountMoveLine(models.Model):
    ...
    l10n_mx_edi_total_discount = fields.Monetary(
        ...
        store=True,
        help="Total discount per invoice line. The formula is discount * quantity of products on the invoice.",
    )
    ...
    def _compute_l10n_mx_edi_amount_discount(self):
        ...
        for line in self:
            discount = float_round(
                float_round(line.price_unit, precision_digits=sale_price_digits)
                * float_round(line.discount, precision_digits=discount_digits)
                / 100.0,
    ...
    def onchange_l10n_mx_edi_amount_discount(self):
        ...
        for line in self:
            if not line.price_unit or not line.quantity:
                continue
            total_discount = line.l10n_mx_edi_amount_discount * float_round(
                line.quantity, precision_digits=discount_digits
            )
            discount = float_round(
                float_round(line.l10n_mx_edi_total_discount, precision_digits=discount_digits)
                / float_round(line.quantity, precision_digits=discount_digits),
                precision_digits=discount_digits,
            )
            line.l10n_mx_edi_total_discount = total_discount
            line.discount = float_round(discount / line.price_unit * 100, precision_digits=discount_digits)
    ...
    def _inverse_discount(self):
        ...
        for line in self.filtered("price_unit"):
            discount = float_round(
                (
                    float_round(line.l10n_mx_edi_amount_discount, precision_digits=digits)
                    / float_round(line.price_unit, precision_digits=digits)
                )
                * 100.00,
...
class AccountMove(models.Model):
    ...
    def _compute_discount(self):
        for inv in self:
            discount = sum(inv.invoice_line_ids.mapped("l10n_mx_edi_total_discount"))
