...
class ProductProduct(models.Model):
    ...
    def _search_quantity_unreserved(self, operator, value):
        """Update _search method for unreserved quantities."""
        if operator not in OPERATORS:
            raise UserError(_("Invalid domain operator %s") % operator)
        if not isinstance(value, (float, int)):
            raise UserError(_("Invalid domain right operand %s") % value)

        # Efficiently fetch product IDs that match the operator
        product_data = self._compute_product_available_not_res_dict()
        matching_ids = [
            product_id for product_id, data in product_data.items()
            if OPERATORS[operator](data["qty_available_not_res"], value)
        ]
