...
class ResPartnerBlacklist(models.Model):
    ...
    def download_csv_partner_list(self):
        ...
        try:
            list_file = requests.get(url, timeout=20)
            number_line = 0
            for count, line in enumerate(list_file.iter_lines()):
                ...
                if line and line[line.index(b",") :] not in file_content and count not in [0, 1] and line != b"":
                    number_line += 1
            ...
            _logger.info(_("Unexpected: %(error)s Number Line: %(line)s", error=str(e), line=str(number_line)))
    ...
    def definitive_partner_list(self, attachment_id):
        ...
        try:
            ...
            _logger.info(_("Unexpected: %s", str(e)))
