...
class Test3rdParty(TestMxEdiCommon):
    def setUp(self):
        ...
        isr_tag = self.env["account.account.tag"].search([("name", "=", "ISR")])
    ...
    def create_invoice_line_to_3rd(self, invoice, product):
        ...
        line = invoice_line._convert_to_write({name: invoice_line[name] for name in invoice_line._cache})
    ...
    def test_xml_node(self):
        ...
        self.assertEqual(len(xml.Conceptos.Concepto), 4, "There should be exactly four nodes 'Concepto'")
        # Retrieve nodes <PorCuentadeTerceros> from all concepts
        node0 = xml.Conceptos.Concepto[0].find(
            "cfdi:ComplementoConcepto/terceros:PorCuentadeTerceros", namespaces=self.namespaces
        )
        ...
        nodes_expected = xml_expected.findall("terceros:PorCuentadeTerceros", namespaces=self.namespaces)
        self.assertXmlTreeEqual(node1, nodes_expected[0])
        self.assertXmlTreeEqual(node2, nodes_expected[1])
        self.assertXmlTreeEqual(node3, nodes_expected[2])
