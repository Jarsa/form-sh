...
class AccountPayment(models.Model):
    ...
    l10n_mx_edi_factoring_id = fields.Many2one(
        "res.partner", "Financial Factor", copy=False, help="This payment was received from this factoring."
    )
    ...
    def default_get(self, fields_list):
        ...
        invoices = (
            ...
            .browse(active_ids)
            .filtered(
                lambda move: move.is_invoice(include_receipts=True) and move.l10n_mx_edi_cfdi_request == "on_invoice"
            )
        )
        if not invoices:
            return rec
        ...
        if not factoring and rec.get("partner_id"):
            factoring = self.env["res.partner"].browse(rec.get("partner_id")).l10n_mx_edi_factoring_id
...
class AccountRegisterPayments(models.TransientModel):
    ...
    def default_get(self, fields_list):
        ...
        if not factoring and rec.get("partner_id"):
            factoring = self.env["res.partner"].browse(rec.get("partner_id")).l10n_mx_edi_factoring_id.ids
