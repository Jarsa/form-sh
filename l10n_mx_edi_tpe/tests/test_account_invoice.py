...
class TestL10nMxEdiInvoiceTPE(TestMxEdiCommon):
    def test_l10n_mx_edi_invoice_tpe(self):
        ...
        self.env["base.language.install"].create({"lang": "es_MX", "overwrite": 0}).lang_install()
        self.product.sudo().write({"l10n_mx_edi_tpe_track": "seaway"})
        invoice = self.invoice
        invoice.partner_id.sudo().write({"country_id": self.ref("base.aw"), "ref": "passport|EGA93812273-PLM3821"})
        ...
        xml_tpe = xml.Complemento.xpath("//tpe:TuristaPasajeroExtranjero", namespaces=namespace)
