...
class SaleOrderAttachments(http.Controller):
    @http.route(
        ["/purchase/order_attachments/<int:order_id>"], type="http", auth="user", methods=["POST"], website=True
    )
    def attach_files(self, order_id, **post):
        ...
        att_ids = purchase_obj.insert_attachment("purchase.order", order_id, post, filename)
