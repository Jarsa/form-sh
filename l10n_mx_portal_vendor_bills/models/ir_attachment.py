...
class IrAttachment(models.Model):
    ...
    def parse_xml(self, xml_file):
        ...
        res = self.sudo().check_xml({xml_file.filename: data})
    ...
    def check_xml(self, files):
        ...
        for key, xml64 in files.items():
            try:
                ...
                wrongfiles.update(
                    {key: {"xml64": xml64, "where": "CheckXML", "error": [exce.__class__.__name__, str(exce)]}}
                )
                continue
    ...
    def validate_documents(self, key, xml, account_id):
        ...
        if xml_type_of_document == "E" and hasattr(xml, "CfdiRelacionados"):
            ...
            related_invoice = xml_related_uuid in inv_obj.search(
                [("l10n_mx_edi_cfdi_name", "!=", False), ("type", "=", "in_invoice")]
        ...
        errors = [
            ...
            ((not inv_id and exist_reference), {"invoice_payment_ref": (xml_name_supplier, xml_serie_folio)}),
            ...
            ((inv_id and inv_folio not in [xml_serie_folio, xml_folio]), {"folio": (xml_serie_folio, inv_folio)}),
            ...
            (
                (not omit_cfdi_related and xml_type_of_document == "E" and not xml_related_uuid),
        ...
        if not inv_id:
            invoice_status = self.create_invoice(
                xml, exist_supplier, exist_currency, xml_taxes.get("taxes_ids", {}), account_id
            )
        ...
        if not float_is_zero(float(inv.amount_total) - float(xml_amount), precision_digits=0):
            inv.message_post(
                body=_(
                    "The XML attached total amount is different to "
                    "the total amount in this invoice. The XML total amount is %s",
                    xml_amount,
                )
            )
        return {"wrongfiles": wrongfiles, "invoices": invoices}
    ...
    def get_local_taxes(self, xml):
        ...
        if hasattr(local_taxes, "RetencionesLocales"):
            for local_ret in local_taxes.RetencionesLocales:
                ...
                tax = tax_obj.search(
                    ["&", ("type_tax_use", "=", type_tax_use), "|", ("name", "=", name), ("amount", "=", tasa)],
                    limit=1,
                )
        ...
        for index, rec in enumerate(xml.Conceptos.Concepto):
            ...
            for tax in taxes:
                ...
                if not tax_account and tax.get("tax", "") not in taxes_to_omit:
                    taxes_list["withno_account"].append(name if name else tax["tax"])
                else:
                    tax["id"] = tax_get.id
