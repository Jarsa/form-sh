...
class TestL10nMxEdiUUID(TransactionCase):
    def setUp(self):
        ...
        self.xml_expected_str = (
            tools.file_open(os.path.join("l10n_mx_edi_uuid", "tests", "expected_cfdi33.xml")).read().encode("UTF-8")
        )
    ...
    def assign_xml_attachment(self, model, uuid="100a0000-2000b-3000c-4000d-50000e060008"):
        ...
        cfdi_filename = ("%s-%s-MX-Invoice-3.3.xml" % (model.journal_id.code, model.payment_reference)).replace(
            "/", ""
        )
    ...
    def test_uuid_attachment_update(self):
        ...
        self.assertEqual(attachment.l10n_mx_edi_cfdi_uuid, uuid, "The UUID must not change manually, only by compute")
        attachment.write({"res_model": "account.account"})
        attachment.refresh()
        self.assertFalse(attachment.l10n_mx_edi_cfdi_uuid, "The UUID of attachment should be empty")
    ...
    def test_uuid_on_invoice_multiple_attachments(self):
        ...
        self.assertTrue(uuid, "The UUID must be filled")
        self.assertEqual(
            invoice.l10n_mx_edi_cfdi_uuid, uuid, "The invoice should have the same UUID as it's attachment"
        )
        ...
        self.assertEqual(
            invoice_2.l10n_mx_edi_cfdi_uuid, uuid_2, "The invoice UUID must be the same of the latest attachment"
        )
    ...
    def test_uuid_on_payment_multiple_attachments(self):
        ...
        self.assertTrue(uuid, "The UUID must be filled")
        self.assertEqual(
            payment.l10n_mx_edi_cfdi_uuid, uuid, "The payment should have the same UUID as it's attachment"
        )
        ...
        attachment.write({"res_id": signed_edi_2.id})
        self.assertEqual(
            payment_2.l10n_mx_edi_cfdi_uuid, uuid_2, "The payment UUID must be the same of the latest attachment"
        )
    ...
    def test_uuid_invoice_duplicated(self):
        ...
        self.env.cr.execute("UPDATE ir_attachment SET res_id = %s WHERE id=%s", (invoice_2.id, attachment.id))
    ...
    def test_uuid_invoice_self_signed(self):
        ...
        # Force a duplicated from sql to bypass odoo constraint
        self.env.cr.execute("UPDATE ir_attachment SET res_id = %s WHERE id=%s", (invoice_3.id, attachment.id))
        with self.assertRaisesRegex(ValidationError, "UUID duplicated"):
            # Check with exists manually is raised
            invoice_3._check_uuid_duplicated()
