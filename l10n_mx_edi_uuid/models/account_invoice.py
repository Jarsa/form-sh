...
class AccountMove(models.Model):
    ...
    l10n_mx_edi_cfdi_uuid = fields.Char(
        compute="_compute_l10n_mx_edi_cfdi_uuid", search="_search_l10n_mx_edi_cfdi_uuid"
    )
    ...
    def _name_search(self, name="", args=None, operator="ilike", limit=100, name_get_uid=None):
        ...
        uuid_domain = self._search_l10n_mx_edi_cfdi_uuid(operator="=", value=name)
    ...
    def _get_falsy_l10n_mx_edi_cfdi_uuid(self, search_domain=None):
        ...
        matched_invoice_ids = [inv_id for inv_id, uuid in inv_uuid.items() if uuid]
    ...
    def _search_l10n_mx_edi_cfdi_uuid(self, operator, value):
        ...
        domain_op_falsy = "in" if operator in NEGATIVE_TERM_OPERATORS else "not in"
        if not value:
            invoice_ids = self._get_falsy_l10n_mx_edi_cfdi_uuid()
            domain = [("id", domain_op_falsy, invoice_ids)]
            return domain
        positive_operator = TERM_OPERATORS_POSITIVE[operator] if operator in NEGATIVE_TERM_OPERATORS else operator
    ...
    def _check_uuid_duplicated(self):
        ...
        mx_invoices = invoices.filtered(lambda r: r.company_id.country_id == self.env.ref("base.mx"))
        ...
        for uuid, record_ids in res.items():
            ...
            msg += _("UUID duplicated %s for following invoices:\n%s\n") % (
                uuid,
                "\n".join(["\t* %d: %s" % (rid, rname) for rid, rname in records.name_get()]),
            )
        if msg:
