...
class IrAttachment(models.Model):
    ...
    l10n_mx_edi_cfdi_uuid = fields.Char(string="Fiscal Folio", index=True, prefetch=False, readonly=True)
    ...
    def update_uuid(self, check_duplicated=True):
        ...
        for attach in uuid_attachments.with_context(prefetch_fields=False):
            if not attach.datas:
                ...
                continue
            cfdi = base64.decodebytes(attach.datas).replace(b"xmlns:schemaLocation", b"xsi:schemaLocation")
        ...
        (self - uuid_attachments + attachments_skipped).with_context(force_l10n_mx_edi_cfdi_uuid=True).write(
