...
class TestL10nMxEdiInvoiceFuel(TestMxEdiCommon):
    def setUp(self):
        ...
        self.xml_expected_ecc = (
            misc.file_open(os.path.join("l10n_mx_edi_fuel", "tests", "expected_ecc.xml")).read().encode("UTF-8")
        )
        self.xml_expected_cc = (
            misc.file_open(os.path.join("l10n_mx_edi_fuel", "tests", "expected_cc.xml")).read().encode("UTF-8")
        )
        self.xml_expected_only_cc = (
            misc.file_open(os.path.join("l10n_mx_edi_fuel", "tests", "expected_only_cc.xml")).read().encode("UTF-8")
        )
        self.xml_expected_ecc_discount = (
            misc.file_open(os.path.join("l10n_mx_edi_fuel", "tests", "expected_ecc_discount.xml"))
            .read()
            .encode("UTF-8")
        )
    ...
    def create_fuel_invoice(self, service_station=None, inv_type="out_invoice", currency_id=None):
        ...
        account_payment = self.env["res.partner.bank"].create(
            {"acc_number": "123456789", "partner_id": self.partner_a.id}
        )
    ...
    def test_l10n_mx_edi_ecc_invoice(self):
        self.invoice.company_id.write({"l10n_mx_edi_isepi": True, "zip": "58000"})
        self.partner_chinaexport.sudo().parent_id.write(
            {
                "ref": "0000123",
                "vat": "XEXX010101000",
        ...
        self.partner_a.commercial_partner_id.write({"vat": "EKU9003173C9", "country_id": self.env.ref("base.mx").id})

        invoice = self.create_fuel_invoice(self.service_station)
        invoice.partner_id = self.partner_chinaexport.parent_id
        invoice.partner_id.sudo().write({"zip": "58000", "l10n_mx_edi_fiscal_regime": "616"})
        invoice.partner_bank_id = self.service_station.bank_ids.id
        invoice.action_post()
        generated_files = (
            self.env["account.edi.document"]
            .sudo()
            .with_context(edi_test_mode=False)
            .search([("state", "in", ("to_send", "to_cancel"))])
        ...
        xml_expected.attrib["TipoCambio"] = xml.attrib["TipoCambio"]
        xml_expected.CfdiRelacionados.CfdiRelacionado.attrib["UUID"] = xml.CfdiRelacionados.CfdiRelacionado.attrib[
            "UUID"
        ]
        self.assertXmlTreeEqual(xml, xml_expected)

    def test_l10n_mx_edi_ecc_invoice_discount(self):
