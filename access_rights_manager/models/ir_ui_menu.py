...
class ir_ui_menu(models.Model):
    ...
    def search(self, args, offset=0, limit=None, order=None):
        ...
        user = self.env.user
        cids = request.httprequest.cookies.get('cids') and int(request.httprequest.cookies.get('cids').split('-')[0]) or self.env.company.id
        for menu_id in user.access_rights_management_ids.filtered(lambda line: cids in line.company_ids.ids).mapped('hide_menu_ids.menu_id'):
