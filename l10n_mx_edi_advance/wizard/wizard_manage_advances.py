    ...
    def default_get(self, fields_values):
        ...
        advances = [
            (
                ...
                0,
                {
                    ...
                    "amount": adv.amount,
                    "amount_available": (
                        adv.invoice_id.l10n_mx_edi_amount_available
                        if adv.invoice_id.currency_id == invoice.currency_id
                        else adv.invoice_id.currency_id._convert(
                            abs(adv.invoice_id.l10n_mx_edi_amount_available),
                            invoice.currency_id,
                            invoice.company_id,
                            adv.invoice_date or fields.Date.today(),
                        )
                    )
                    + adv.amount,
            ...
            for adv in invoice.l10n_mx_edi_advance_ids
        ]
        ...
        for adv in invoice._get_outstanding_advances():
            if adv in invoice.l10n_mx_edi_advance_ids.mapped("advance_id"):
                continue
            available = (
                adv.amount_available
                if adv.currency_id == invoice.currency_id
                else (
                    adv.currency_id._convert(
                        abs(adv.amount_available),
                        invoice.currency_id,
                        invoice.company_id,
                        adv.date or fields.Date.today(),
                    )
                )
            )
            advances.extend(
                [
                    (
                        ...
                        0,
                        {
                            "advance_id": adv.id,
                            "amount": 0
                            if invoice.l10n_mx_edi_advance_ids
                            else available
                            if amount_in_advances + available <= invoice.amount_total
                            else max(0, invoice.amount_total - amount_in_advances),
    ...
    def apply_advances(self):
        ...
        for advance in self.advance_ids:
            ...
            if advance.amount > advance.amount_available + advance_line.amount:
                raise UserError(
                    _(
                        "The amount cannot be bigger than available amount. %(amount)s > %(available)s",
                        amount=round(advance.amount, 2),
                        available=round(advance.amount_available + advance_line.amount, 2),
                    )
                )
            if advance_line:
            ...
            if not advance.amount:
                continue
            invoice.l10n_mx_edi_advance_ids = [(0, 0, {"advance_id": advance.advance_id.id, "amount": advance.amount})]
...
class InvoiceApplyAdvancesDetail(models.TransientModel):
    ...
    def _compute_amount_available(self):
        ...
        for line in self - with_advance:
            line.amount_available = 0
        for line in with_advance:
            amount_available = (
                line.advance_id.amount_available
                + invoice.l10n_mx_edi_advance_ids.filtered(lambda a: a.advance_id == line.advance_id).amount
            )
            line.amount_available = (
                amount_available
                if (line.advance_id.currency_id == invoice.currency_id)
                else line.advance_id.currency_id._convert(
                    abs(amount_available),
                    invoice.currency_id,
                    invoice.company_id,
