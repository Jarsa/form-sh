...
class AccountMove(models.Model):
    ...
    def _compute_amount_advances(self):
        advances_a = self.filtered(
            lambda i: i._l10n_mx_edi_get_advance_case() == "A"
            and i.move_type == "out_invoice"
            and i.l10n_mx_edi_origin
            and i.l10n_mx_edi_origin.startswith("07")
        )
        ...
        for record in advances_a:
            if record.state == "draft":
                ...
                credit_note = reverse_entries.filtered(
                    lambda i: i._l10n_mx_edi_get_advance_uuid_related() and i.state not in ("draft", "cancel")
                )
    ...
    l10n_mx_edi_amount_residual_advances = fields.Monetary(
        "Amount residual with advances",
        compute="_compute_amount_advances",
        help="Save the amount that will be applied as advance when validate the invoice",
    )
    ...
    l10n_mx_edi_advance_ids = fields.One2many(
        "l10n_mx_edi.advance.detail",
        "invoice_id",
        "Advances applied",
        copy=False,
        help="Detail for the advances applied to this invoice.",
    )
    l10n_mx_edi_has_outstanding_advances = fields.Boolean(
        ...
        "Has Advances?",
        compute="_compute_get_outstanding_info",
        help="This field will be True if the customer in the invoice has advances that could be used on the customer "
        "invoice.",
    )
    # Fields for advances
    l10n_mx_edi_is_advance = fields.Boolean(
        "Is Advance?", compute="_compute_is_advance", store=True, help="Indicates if this invoice is an advance."
    )
    ...
    def _compute_advance_available(self):
        advances = self.filtered(
            lambda a: a.state == "posted" and a.payment_state in ("paid", "in_payment") and a._l10n_mx_edi_is_advance()
        )
        ...
        for record in advances:
            ...
            for inv in (
                record.l10n_mx_edi_related_document_ids_inverse.filtered(lambda inv: inv.state != "cancel")
                .mapped("l10n_mx_edi_advance_ids")
                .filtered(lambda a: a.advance_id.name == record.l10n_mx_edi_cfdi_uuid)
            ):
                available -= (
                    inv.amount
                    if inv.invoice_id.currency_id == record.currency_id
                    else inv.invoice_id.currency_id._convert(
                        inv.amount,
                        record.currency_id,
                        record.company_id,
    ...
    def _get_outstanding_advances(self):
        ...
        domain = [
            "|",
            ("partner_historical_id", "in", [self.partner_id.id, financial_partner.id]),
    ...
    def _compute_get_outstanding_info(self):
        for record in self:
            if (
                ...
                or (not record._get_outstanding_advances() and not record.l10n_mx_edi_advance_ids)
            ):
    ...
    def _onchange_invoice_line_ids(self):
        ...
        self.update(
            {
                "invoice_payment_term_id": self.env.ref("account.account_payment_term_immediate"),
    ...
    def action_post(self):
        ...
        """
        has_advance = self.filtered(
            lambda r: r.move_type == "out_invoice"
            and r._l10n_mx_edi_get_advance_case() == "A"
    ...
    def l10n_mx_edi_advance_sign_refund(self):
        ...
        self.l10n_mx_edi_origin = (
            (self.l10n_mx_edi_origin or "") + self.reversed_entry_id.l10n_mx_edi_cfdi_uuid
            if self.l10n_mx_edi_origin == "07|"
            else "07|%s" % self.reversed_entry_id.l10n_mx_edi_cfdi_uuid
        )
    ...
    def _l10n_mx_edi_prepare_advance_cron(self):
        """To avoid disable the after commit, the credit note will be send
        to the SAT after of the invoice validation
        """
                ...
                [
                    (
                        ...
                        0,
                        {
                            ...
                            "analytic_account_id": advance.mapped(
                                "advance_id.advance_id.invoice_line_ids.analytic_account_id").id,
                            "analytic_tag_ids": [(6, 0, advance.mapped(
                                "advance_id.advance_id.invoice_line_ids.analytic_tag_ids").ids)],
                        },
                    )
                ]
            )
