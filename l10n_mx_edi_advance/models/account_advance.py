...
class L10nMXEdiAdvance(models.Model):
    ...
    name = fields.Char(
        compute="_compute_advance_fields",
        store=True,
        compute_sudo=True,
    ...
    uuid = fields.Char(
        "UUID", help="If the advance do not exists in Odoo, set here the UUID to be set in the invoice."
    )
    ...
    partner_id = fields.Many2one(
        "res.partner", compute="_compute_advance_fields", store=True, compute_sudo=True, help="Partner in the advance."
    )
    ...
    advance_total = fields.Monetary(
        compute="_compute_advance_fields", compute_sudo=True, help="Amount total in the advance related."
    )
    amount_available = fields.Monetary(
        compute="_compute_amount_total", store=True, help="Advance available for this advance."
    )
    ...
    is_historical = fields.Boolean(
        "Is Historical?", default=True, tracking=True, help="Mark this option if the advance invoice is not in Odoo."
    )
        ...
        for rec in self.filtered("uuid"):
            if not len(rec.uuid) == 36 or not re.match(pattern, rec.uuid):
                raise ValidationError(
                    _(
                        "The number: %s doesn't match the pattern of a CFDI we are unable to look "
                        "for this related document.",
                        rec.uuid,
                    )
                )
    ...
    @api.depends(
        ...
        "partner_historical_id",
        "currency_historical_id",
        "advance_subtotal",
    )
    ...
    @api.depends(
        "advance_id.l10n_mx_edi_amount_available",
        "invoice_ids",
        "invoice_ids.invoice_id.state",
        "advance_historical_total",
    )
