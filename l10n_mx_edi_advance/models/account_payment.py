...
class AccountPayment(models.Model):
    ...
    def l10n_mx_edi_advance_is_required(self, amount):
        ...
        self.ensure_one()
        avoid_advance = (
            self.env["ir.config_parameter"].sudo().get_param("l10n_mx_edi_advance_avoid_advance_on_payment")
        )
        if any(
            [
                not self.l10n_mx_edi_generate_advance,
                avoid_advance,
                not self.partner_id,
        ...
        amount_debt = company.currency_id.round(lines[0]["debit"] - (lines[0]["credit"])) if lines else 0.0
        if amount_debt == amount * -1:
            return False
        if debt > 0:
            messages.append(
                _("This payment do not generate advance because the customer has invoices with pending payment.")
            )
        if not messages:
            return self._context.get("payment_difference") or debt or amount
        self.message_post(
            body=_(
                "This record cannot create the advance document automatically for the next reason: %sFor this record, "
                "you need create the invoice manually and reconcile it with this payment or cancel and validate "
                "again after that the data was completed.",
                create_list_html(messages),
            )
        )
        return False
    ...
    def _l10n_mx_edi_generate_advance(self, amount):
        ...
        edi_document = advance.edi_document_ids.filtered(lambda d: d.edi_format_id == cfdi_v33)
