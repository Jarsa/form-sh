...
class TestMxEdiAdvanceInvoice(AdvanceTransactionCase):
    ...
    def test_002_create_advance_multi_currency(self):
        ...
        self.assertFalse(
            refund.currency_id.compare_amounts(refund_amount_total, advance_amount_total),
    ...
    def test_004_create_advance_from_payment_with_stamp_errors(self):
        ...
        partner = self.partner_a.copy({"name": "ADV", "parent_id": False})
        # CFDI error
    ...
    def test_005_advance_amounts_fields(self):
        ...
        self.assertFalse(
            float_compare(invoice.l10n_mx_edi_amount_advances, adv_amount, precision_digits=0),
            "Advance amount is failing in draft state",
        )
        self.assertFalse(
            float_compare(
                invoice.l10n_mx_edi_amount_residual_advances, invoice.amount_total - adv_amount, precision_digits=0
            ),
            "Advance Residual amount is failing in draft state",
        )
        invoice.action_post()
        invoice.l10n_mx_edi_cfdi_uuid = "TEST"
        invoice.search([("reversed_entry_id", "=", invoice.id)]).l10n_mx_edi_advance_sign_refund()
    ...
    def test_006_advance_amounts_fields_multicurrency(self):
        ...
        self.assertFalse(
            invoice.currency_id.compare_amounts(round(invoice.l10n_mx_edi_amount_advances, 0), round(adv_amount, 0)),
            "Advance amount is failing in draft state %s != %s" % (invoice.l10n_mx_edi_amount_advances, adv_amount),
        )
        self.assertFalse(
            invoice.currency_id.compare_amounts(
        ...
        self.assertFalse(
            invoice.currency_id.compare_amounts(invoice.l10n_mx_edi_amount_residual_advances, invoice.amount_residual),
            "Advance residual amount is failing in draft state %s != %s"
        ...
        self.assertFalse(
            invoice.currency_id.compare_amounts(invoice.l10n_mx_edi_amount_advances, adv_amount),
            "Advance amount is failing in draft state %s != %s" % (invoice.l10n_mx_edi_amount_advances, adv_amount),
        )
        self.assertFalse(
            invoice.currency_id.compare_amounts(
        ...
        self.assertFalse(
            float_compare(
                invoice.l10n_mx_edi_amount_advances, invoice.amount_total - invoice.amount_residual, precision_digits=0
            ),
            "Advance amount is failing in draft state %s != %s"
            % (invoice.l10n_mx_edi_amount_advances, invoice.amount_total - invoice.amount_residual),
        )
        self.assertFalse(
            float_compare(invoice.l10n_mx_edi_amount_residual_advances, invoice.amount_residual, precision_digits=0),
            "Advance residual amount is failing in draft state %s != %s"
            % (invoice.l10n_mx_edi_amount_residual_advances, invoice.amount_residual),
        )
    ...
    def test_008_historical_advance(self):
        ...
        self.assertEqual(
            advance.amount_available,
            advance.advance_historical_total,
    ...
    def test_010_advance_refund(self):
        ...
        refund = (
            ...
            .with_context(active_id=advance.id, active_ids=advance.ids, active_model="account.move")
            .create(
                {
