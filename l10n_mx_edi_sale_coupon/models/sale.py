...
class SaleOrder(models.Model):
    ...
    def _create_invoices(self, grouped=False, final=False, date=None):
        ...
        if not (
            self.mapped("applied_coupon_ids")
            or self.mapped("no_code_promo_program_ids")
            or self.mapped("code_promo_program_id")
        ):
            return moves
...
class SaleOrderLine(models.Model):
    ...
    def _prepare_invoice_line(self, **optional_values):
        ...
        if self.is_reward_line and program.filtered(
            lambda p: p.discount_apply_on in ("on_order", "specific_products") or p.discount_type == "fixed_amount"
        ):
            res.update(
                {
                    "name": _(
                        "%(name)s\nTotal Discount:%(price)s\nCoupon:%(coupon)s",
                        name=res.get("name"),
                        price=res.get("price_unit", 0),
                        coupon=order.applied_coupon_ids.display_name or "",
                    ),
                    "price_unit": 0,
                }
            )
        ...
        if (
            ...
            and program
            and program == program.filtered(lambda p: p.discount_type == "fixed_amount")
        ):
            ...
            for fixed_program in program.filtered(lambda p: p._get_valid_products(self.product_id)):
                ...
                tax = self.tax_id.filtered(
                    lambda t, tax_group_id=tax_group_id: t.type_tax_use == "sale"
                    and t.amount_type == "percent"
                    and t.tax_group_id in tax_group_id
                )
                if tax:
        ...
        elif not self.is_reward_line and program.filtered(
            lambda p: p.discount_apply_on == "on_order" or p.discount_type == "fixed_amount"
        ):
            ...
            lambda p: p.discount_apply_on == "specific_products" and self.product_id in p.discount_specific_product_ids
        ):
            ...
            factor = (
                abs(sum(reward.mapped("price_total"))) / sum(lines.mapped("price_total")) if self.price_total else 1
            )
            res.update(
                {
                    "l10n_mx_edi_amount_discount": factor * self.price_unit,
