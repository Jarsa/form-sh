...
class AccountMove(models.Model):
    ...
    def _prepare_sale_move(self, move_vals):
        ...
        amount = current_currency._convert(
            move_vals["amount_total"], company_currency, asset_id.company_id, depreciation_date
        )
        value = current_currency._convert(
            asset_id.original_value, company_currency, asset_id.company_id, depreciation_date
        )
        ...
        move_line_2 = {
            ...
            "debit": (value - amount) if float_compare(value - amount, 0.0, precision_digits=prec) > 0 else 0.0,
            "credit": 0.0 if float_compare(value - amount, 0.0, precision_digits=prec) > 0 else -(value - amount),
            "analytic_account_id": asset_id.asset_type == "sale" and account_analytic_id.id,
            "analytic_tag_ids": asset_id.asset_type == "sale" and [(6, 0, analytic_tag_ids.ids)],
            "currency_id": company_currency != current_currency and current_currency.id,
            "amount_currency": (asset_id.original_value - move_vals["amount_total"])
    ...
    def create_sale_move(self, vals, post_move=True):
        ...
        if not asset_id.account_cogs_id:
            raise UserError(
                _("COGS account of asset category needs to be configured. Please check the asset category.")
            )
