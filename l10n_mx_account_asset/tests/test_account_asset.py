...
class TestL10nMxAccountAsset(TestMxEdiCommon):
    ...
    def test_00_account_asset_asset(self):
        ...
        asset.validate()
        self.assertEqual(
            asset.method_number, len(asset.depreciation_move_ids), "Depreciation lines not created correctly"
        )
        ...
        cogs_line = sale_move.mapped("line_ids").filtered(lambda a: a.account_id == asset.account_cogs_id)
        self.assertTrue(cogs_line)
        self.assertEqual(cogs_line.debit or cogs_line.credit, value_residual)

        # There is a line in the move that contains the asset's Asset Account
        # with the gross value of the asset
        asset_line = sale_move.mapped("line_ids").filtered(lambda a: a.account_id == asset.account_asset_id)
        ...
        depreciation_line = sale_move.mapped("line_ids").filtered(
            lambda a: a.account_id == asset.account_depreciation_id
        )
        self.assertTrue(depreciation_line)
        self.assertEqual(depreciation_line.debit or depreciation_line.credit, accum_depreciation)
