...
class MxReportJournalEntries(models.AbstractModel):
    ...
    def group_by_journal_id(self, options, line_id):
        ...
        if options.get("move_name"):
            move = entry_obj.search([("ref", "ilike", options.get("move_name"))])
        ...
        for journal in journal_ids:
            ...
            if context.get("analytic_tag_ids"):
                domain += [
                    "|",
                    ("analytic_account_id.tag_ids", "in", context["analytic_tag_ids"].ids),
            ...
            if not context.get("print_mode"):
                move_ids = [m["move_id"][0] for m in move_obj.read_group(domain, ["move_id"], "move_id", limit=81)]
            else:
                move_ids = [m["move_id"][0] for m in move_obj.read_group(domain, ["move_id"], "move_id")]
            journals[journal]["lines"] = entry_obj.search([("id", "in", move_ids)], order="date")
        return journals

    def _get_lines(self, options, line_id=None):
        # update filter_date to get report name
        self.filter_date.update(
            {"date_to": options.get("date").get("date_to"), "date_from": options.get("date").get("date_from")}
        )

        lines = []
        line_id = int(line_id.split("_")[1]) if line_id else None
        if line_id:
            lines.extend(self._get_lines_second_level(options, self.env["account.move"].browse(line_id)))
            return lines
        context = self.env.context
        ...
        grouped_journals = self.with_context(
            ...
            date_from=dt_from
            and company_id.compute_fiscalyear_dates(datetime.strptime(dt_from, "%Y-%m-%d"))["date_from"]
            or None,
    ...
    def _get_caret_type(self, line):
        ...
        if "invoice" in line.move_id.move_type:
            caret_type = (
                "account.move.in" if line.move_id.move_type in ("in_refund", "in_invoice") else "account.move.out"
            )
    ...
    def get_bce_dict(self, options):
        ...
        date = fields.datetime.strptime(self.env.context["date_from"], DEFAULT_SERVER_DATE_FORMAT)
        basis_account_ids = moves.mapped("company_id.account_cash_basis_base_account_id")
        moves_to_exclude = [
            value.move_id.id
            for value in moves.mapped("line_ids").filtered(lambda m: m.account_id in basis_account_ids)
        ]
        chart = {
    ...
    def _get_report_name(self):
        ...
        date_report = (
            fields.datetime.strptime(self.filter_date.get("date_to"), "%Y-%m-%d")
            if self.filter_date.get("date_to", False)
            else fields.date.today()
        )
        name = "%s%s%sPL" % (self.env.user.company_id.vat or "", date_report.year, str(date_report.month).zfill(2))
        return name
