...
class TestJournalEntryReport(TestMxEdiCommon):
    def setUp(self):
        ...
        self.xml_expected_str = (
            misc.file_open(join("l10n_mx_edi_reports", "tests", "expected_moves.xml")).read().encode("UTF-8")
        )
        self.payment_method_manual_out = self.env.ref("account.account_payment_method_manual_out")
        self.transfer = self.env.ref("l10n_mx_edi.payment_method_transferencia")
        self.bank_account = (
            self.env.ref("account_bank_statement_import.ofx_partner_bank_1")
            .sudo()
            .copy({"company_id": self.invoice.company_id.id})
        )
