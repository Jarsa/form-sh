...
class TestL10nMxEdiInvoiceImpLocal(TestMxEdiCommon):
    def setUp(self):
        ...
        for rep_line in (
            self.tax_10_negative.invoice_repartition_line_ids | self.tax_10_negative.refund_repartition_line_ids
        ):
        ...
        self.xml_expected = (
            misc.file_open(os.path.join("l10n_mx_edi_implocal", "tests", "expected.xml")).read().encode("UTF-8")
        )
    ...
    def test_l10n_mx_edi_implocal(self):
        ...
        comp = xml.Complemento.xpath("//implocal:ImpuestosLocales", namespaces=namespaces)
